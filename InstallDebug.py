...
deb = input("Digite 's' para colocar modo Debbug \n Digite 'n' para modo normal e ocultar o cmd antes de abrir a execucao: \n")
deb = 's'
"""
...
#Liga o Debug e o Console para Testar Aplicação
AtuSpec = Spec.replace("debug=False", "debug=True")
os.remove( os.path.join ( os.getcwd(), str(py_principal).replace(".py", ".spec") ) )
open( str(py_principal).replace(".py", ".spec")  , 'w'  ).write(AtuSpec)
...
"""
#usar site para baixar dlls automaticamente: https://www.dll-files.com/
...
#Selecionar a unidade do disco atual e pasta atual
bat = bat + str( os.getcwd() ).split(":")[0] + ":" + chr(10)
bat = bat + 'cd "' + str( os.getcwd() ) + '"' + chr(10)

bat = bat + "@call pyInstaller {}{}".format( str(py_principal).replace(".py", ".spec"), chr(10) )
bat = bat + "exit"
